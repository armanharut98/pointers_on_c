...
void rearrange(char *output, char const *input, int n_columns, int const columns[])
{
    ...
    for (col = 0; col < n_columns; col += 2) {
        /* 
        ** find the start and the end of the pair
        */
        int pair_start;
        int pair_end;

        /*
        ** check if pair has an end
        */
        if (columns[col + 1] == -1) {
            pair_start = columns[col];
            pair_end = len - 1;
        } else if (columns[col + 1] > columns[col]) {
            pair_start = columns[col];
            pair_end = columns[col + 1];
        } else {
            pair_start = columns[col + 1];
            pair_end = columns[col];
        }

        int nchars = pair_end - pair_start + 1;

        /*
        ** If the input line isn't this long or 
        ** the output array is full, we're done
        */
        if (pair_end >= len || output_col == MAX_INPUT - 1)
            break;
        ...
        */
        strncpy(output + output_col, input + pair_start, nchars);
        output_col += nchars;
    }
